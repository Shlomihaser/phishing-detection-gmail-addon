    ...
    def evaluate(self, email: Email) -> Optional[DetectorResult]:
        ...
        if email.auth_results:
            ...
            if not spf and not dkim and not dmarc:
                reasons.append(
                    "No authentication headers present (SPF/DKIM/DMARC missing)"
                )
                ...
                if dmarc:
                    if dmarc == "fail":
                        reasons.append(
                            "DMARC policy failed (domain owner rejects this email)"
                        )
                        max_score = max(max_score, 100.0)
        ...
        if email.reply_to and email.sender_email:
            ...
            sender_domain = (
                sender_ext.registered_domain.lower()
                if sender_ext.registered_domain
                else ""
            )
            reply_domain = (
                reply_ext.registered_domain.lower()
                if reply_ext.registered_domain
                else ""
            )
        ...
        if email.return_path and email.sender_email:
            ...
            return_domain = (
                return_ext.registered_domain.lower()
                if return_ext.registered_domain
                else ""
            )
