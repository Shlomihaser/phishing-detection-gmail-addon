...
async def test_scan_endpoint_success(email_builder, mock_ml_service):
    ...
    """
    raw_mime = (email_builder
                .with_sender("user@example.com")
                .with_subject("Safe Email")
                .build())
...
async def test_scan_endpoint_crash_handling(mocker):
    ...
    mocker.patch("app.detectors.links.MaliciousLinkDetector.evaluate", side_effect=Exception("Database Boom"))
    ...
    mocker.patch("app.services.email_parser.EmailParser.parse", side_effect=Exception("Critical Fail"))
